...
export default function ListingDetail() {
  ...
  const existingFavourite = favourites.find(
    (f) => String(f.listing?._id || f.listing) === String(id),
  );
  ...
  const handleSendInquiry = async (e) => {
    e.preventDefault();
    const result = await dispatch(sendInquiry({ listingId: id, message }));
    if (sendInquiry.fulfilled.match(result)) {
      setMessage("");
      setTimeout(() => dispatch(clearInquirySuccess()), 3000);
    }
  };
          ...
          {isOwner && (
            <div className="mt-8">
              <h3 className="font-display text-xl text-ink mb-4">
                Inquiries received{" "}
                {listingInquiries.length > 0 && `(${listingInquiries.length})`}
              </h3>
              ...
              {listingInquiries.length === 0 ? (
                ...
                <div className="space-y-3">
                  {listingInquiries.map((inquiry) => (
                    <div
                      key={inquiry._id}
                      className="bg-white p-4 border border-ink/10"
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium text-ink text-sm">
                          {inquiry.sender?.username || "Unknown"}
                        </span>
            ...
            <>
              <button
                ...
                className={`w-full px-5 py-2.5 text-sm font-medium mb-4 transition-colors ${
                  isFavourited
                    ? "bg-brass text-ink hover:bg-red-100 hover:text-red-700"
                    : "bg-stone border border-brass text-ink hover:bg-brass"
                }`}
              >
                {isFavourited
              ...
              <div className="bg-white p-6 border border-ink/10">
                ...
                {inquiryError && (
                  <p className="text-red-600 text-sm mb-3">{inquiryError}</p>
                )}
